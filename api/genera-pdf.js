...
${isBanca ? `  /* Report Banca: testo risultati in nero */
  ...
  .kpi-val,.pos,.neg,.neu,.warn{color:#000!important;}
  .kpi-box{border-left-color:#94A3B8!important;}` : ''}
...
<div class="cover">
  ...
  <div style="position:relative;z-index:1;">
    ...
    <div class="cv-footer">${isBanca?'Documento tecnico riservato a istituti di credito · Solo valori e indici, senza suggerimenti consulenziali ·':'Documento riservato ·'} AnalisiEBusinessPlan.com · Rating: Z'-Score Altman PMI (30%) + Scorecard EBA/GL/2020/06 (70%) + Trigger CCII</div>
  </div>
</div>
...
<div class="page">
  ...
  <div class="banc-grid">
    ${(()=>{
      const dscrNA=isNaN(c.dscr); const noDebt=(d.rate_cap||0)===0&&(d.interessi||0)===0&&(d.oneri_f||0)===0;
      const dscrLabel=dscrNA?(noDebt?'Non calcolabile: nessun debito finanziario (rata capitale = 0, interessi = 0)':'Non calcolabile: dati insufficienti'):'';
      const icrNA=isNaN(c.icr); const noInt=(d.oneri_f||0)===0&&(d.interessi||0)===0;
      const icrLabel=icrNA?(noInt?'Non calcolabile: oneri finanziari = 0 (nessun debito finanziario)':'Non calcolabile: dati insufficienti'):'';
      return [
        ['DSCR','Debt Service Coverage Ratio','FCO (o EBITDA) / (Quote capitale + Interessi)', c.dscr, dscrNA?'N/A':fx(c.dscr), dscrNA?'bv-0':c.dscr>=1.25?'bv-g':c.dscr>=1.0?'bv-a':'bv-r', dscrNA?dscrLabel:c.dscr>=1.25?'✓ Bancabile ≥ 1,25x':c.dscr>=1.0?'⚠ Borderline':'✗ Stage 2 EBA', dscrNA?null:Math.min(c.dscr/3*100,100), semColor(c.dscr,1.25,1.0), 'Indicatore chiave per le banche. DSCR &lt;1,1 = trigger allerta EBA formale. DSCR &lt;1,0 = i flussi non coprono il servizio del debito.'],
        ...
        ['ICR','Interest Coverage Ratio','EBIT / Oneri Finanziari', c.icr, icrNA?'N/A':fx(c.icr), icrNA?'bv-0':c.icr>=3?'bv-g':c.icr>=1.5?'bv-a':'bv-r', icrNA?icrLabel:c.icr>=3?'✓ Ottimo >3x':c.icr>=1.5?'⚠ Sufficiente':'✗ Critico', icrNA?null:Math.min(c.icr/6*100,100), semColor(c.icr,3,1.5), 'Quante volte il reddito operativo copre gli interessi. ICR &lt;1 = impossibile pagare gli interessi con l\'EBIT.'],
...
<div class="page">
  ...
  <div class="pf"><span>AnalisiEBusinessPlan.com</span><span>${nome} — ${anno}</span><span>Pag. ${hasRF?'8':'7'}</span></div>
</div>
