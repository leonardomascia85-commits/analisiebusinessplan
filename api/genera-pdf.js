...
<div class="page">
  ...
  <div class="banc-grid">
    ${[
      (()=>{ const na=isNaN(c.dscr); const noDebt=(d.rate_cap||0)===0&&(d.interessi||0)===0&&(d.oneri_f||0)===0; const naLabel=na?(noDebt?'Non calcolabile: nessun debito finanziario (rata capitale = 0, interessi = 0)':'Non calcolabile: dati insufficienti'):''; return ['DSCR','Debt Service Coverage Ratio','FCO (o EBITDA) / (Quote capitale + Interessi)', c.dscr, na?'N/A':fx(c.dscr), na?'bv-0':c.dscr>=1.25?'bv-g':c.dscr>=1.0?'bv-a':'bv-r', na?naLabel:c.dscr>=1.25?'✓ Bancabile ≥ 1,25x':c.dscr>=1.0?'⚠ Borderline':'✗ Stage 2 EBA', na?null:Math.min(c.dscr/3*100,100), semColor(c.dscr,1.25,1.0), 'Indicatore chiave per le banche. DSCR &lt;1,1 = trigger allerta EBA formale. DSCR &lt;1,0 = i flussi non coprono il servizio del debito.']; })()
      ...
      (()=>{ const na=isNaN(c.icr); const noInt=(d.oneri_f||0)===0&&(d.interessi||0)===0; const naLabel=na?(noInt?'Non calcolabile: oneri finanziari = 0 (nessun debito finanziario)':'Non calcolabile: dati insufficienti'):''; return ['ICR','Interest Coverage Ratio','EBIT / Oneri Finanziari', c.icr, na?'N/A':fx(c.icr), na?'bv-0':c.icr>=3?'bv-g':c.icr>=1.5?'bv-a':'bv-r', na?naLabel:c.icr>=3?'✓ Ottimo >3x':c.icr>=1.5?'⚠ Sufficiente':'✗ Critico', na?null:Math.min(c.icr/6*100,100), semColor(c.icr,3,1.5), 'Quante volte il reddito operativo copre gli interessi. ICR &lt;1 = impossibile pagare gli interessi con l\'EBIT.']; })()
