...
Prima versione tecnica del nuovo sito "PrimoFisco" (guide + corsi + attestati), deciso in `../ANALISI-PROGETTO-GUIDE-FORMAZIONE.md` (§17-19).

**Stato**: bozza statica (HTML/CSS puro, nessuna dipendenza), pensata per essere migrata sul dominio proprio `primofisco.it` non appena registrato — vive qui solo perché è l'unico repository disponibile in questa sessione.
...
1. Registrare `primofisco.it` e `.com` (§17.4 del documento di analisi).
...
5. Aprire il canale YouTube "PrimoFisco" e iniziare la pubblicazione settimanale (§18.3).
